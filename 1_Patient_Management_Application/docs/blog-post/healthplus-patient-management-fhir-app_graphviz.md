...
![Architecture Overview](visualizations/generated/architecture_overview.svg)
*High-level architecture: Browser SPA communicates directly with HAPI FHIR R4 via REST.*
...
![FHIR Resource ERD](visualizations/generated/fhir_resource_erd.svg)
*Entity-Relationship Diagram showing Patient, Encounter, Appointment, and Condition relationships.*

### CRUD Operations Flow

Every user action in the application maps to one or more FHIR REST operations. This diagram traces the complete lifecycle of a patient record:

![CRUD Sequence](visualizations/generated/crud_sequence.svg)
*CRUD operations: Create (POST), Read/Search (GET), Update (PUT), Delete (DELETE with cascade fallback).*
...
![Lazy Loading Architecture](visualizations/generated/lazy_loading.svg)
*Throttled lazy loading: 6 concurrent workers fetch Encounter/Appointment data after the table renders.*
...
![SPA View Architecture](visualizations/generated/spa_views.svg)
*Application view states and transitions: Home, Patient List, Create/Update Forms, and Delete Modal.*
...
## Generating the SVG Diagrams

To generate the high-resolution SVG diagrams used in this document:

```bash
cd blog-post/visualizations
pip install graphviz
python pma_diagrams.py
```

This produces 5 SVG files in `visualizations/generated/`:
- `architecture_overview.svg` -- High-level architecture
- `fhir_resource_erd.svg` -- Resource relationship diagram
- `crud_sequence.svg` -- CRUD operations flow
- `lazy_loading.svg` -- Lazy loading architecture
- `spa_views.svg` -- SPA view state machine
